...
fn main() {
    ...
    match io::stdin().read_line(&mut rate_input) {
        Ok(_) => {
            match rate_input.trim().parse::<f64>() {
    ...
    match io::stdin().read_line(&mut hours_input) {
        Ok(_) => {
            match hours_input.trim().parse::<f32>() {
    ...
    println!("Increase rate: {}", increase_for_rate / increase_rate_in_hours as f64);

    let mut input: String = String::new();
    let mut per_hour: f64 = 0.0;
    let mut worked_hours: f32 = 0.0;

    println!("Please enter the per hour wage (starting with \"*\") and hours worked:");
    ...
    let final_rate = per_hour + (worked_hours as f64 * increase_for_rate / increase_rate_in_hours as f64);

    println!("Final rate : {}", final_rate);

    println!("To pay : {}", worked_hours as f64 * final_rate);
